...
bool LlamaCppOcrEngine::LoadModel(const std::string& modelPath, const std::string& mmprojPath) {
    ...
    // 2. 初始化 mtmd 视觉多模态 Context (对齐 mtmd-cli.cpp init_vision_context)
...
void LlamaCppOcrEngine::RecognizeStream(
    ...
    m_workerThread = std::thread([this, imagePath, taskType, actualModelPath, actualMmprojPath, onToken, onComplete]() {
        ...
        // 6. 逐 Chunk 编码媒体 Embedding 与解码 (完全参照 mtmd-cli.cpp eval_message 流程 L276-L360)
        llama_pos n_past = 0;
        ...
        for (size_t i = 0; i < n_chunks; i++) {
            ...
            if (chunk_type == MTMD_INPUT_CHUNK_TYPE_TEXT) {
                ...
                // 媒体/图像 Chunk：优先尝试从当前已编码的 mbatch 中获取 Embedding (实现向量复用)
                float* embd = nullptr;
                if (mbatch) {
                    embd = mtmd_batch_get_output_embd(mbatch, chunk);
                }

                // 若未在当前 mbatch 中（缓存未命中），则新建 mbatch 并合并后续媒体 Chunk 批量编码
                if (!embd) {
                    if (mbatch) {
        ...
        // 7. 构建 Sampler 采样链 (严格对齐 mtmd-cli.cpp common_sampler 贪婪采样模式)
        ...
        // 预初始化批处理结构，避免循环内部频繁分配/释放内存 (对齐 common_batch 机制)
        llama_batch batch = llama_batch_init(1, 0, 1);

        bool decode_error = false;

        // 8. 逐 Token 解码推理循环 (严格参照 mtmd-cli.cpp generate_response 流程 L194-L224)
        for (int i = 0; i < maxTokens; i++) {
            ...
            common_sampler_accept(smpl, id, true); // 遵从 AGENTS.md 规范：更新 Sampler 历史

            if (llama_vocab_is_eog(vocab, id)) {
                break;
            }

            // 使用 common_token_to_piece 转为 String 并进行 Control Token 过滤 (对齐 mtmd-cli.cpp L209)
            ...
            // 对齐 mtmd-cli.cpp generate_response 解码流程：复用 batch 并传入递增的 current_n_past++ 位置 (L218-L220)
        ...
        // 参照 mtmd-cli.cpp L226，使用 common_detokenize 完整解码整个生成的 Token 序列，消除流式切片漏字或多字节乱码
