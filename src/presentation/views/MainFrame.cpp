...
MainFrame::MainFrame(
    ...
    : wxFrame(nullptr, wxID_ANY, L"译灵 · LinguaAlpaca", wxDefaultPosition,
              wxSize(1080, 780), wxBORDER_NONE),
...
void MainFrame::InitUI() {
  ...
  wxPanel *logoBadge =
      new wxPanel(m_topHeaderPanel, wxID_ANY, wxDefaultPosition, wxSize(28, 28),
                  wxBORDER_NONE);
  ...
  // 模式切换按钮 (SVG Moon/Sun)
  bool isLight =
      Application::Service::ThemeManager::GetInstance().GetCurrentTheme() ==
      Domain::Model::AppThemeMode::Light;
  wxBitmapBundle themeBundle = Theme::IconManager::GetIconBundle(
      isLight ? Theme::SVG::MOON : Theme::SVG::SUN, wxSize(18, 18),
      palette.textPrimary);
  m_themeBtn =
      new wxBitmapButton(m_topHeaderPanel, wxID_ANY, themeBundle,
                         wxDefaultPosition, wxSize(36, 36), wxBORDER_NONE);
  m_themeBtn->SetBackgroundColour(palette.sidebarBg);

  // 自定义窗口控制按钮 (SVG 最小化, 放大/还原, 关闭)
  wxBitmapBundle minBundle = Theme::IconManager::GetIconBundle(
      Theme::SVG::MINIMIZE, wxSize(14, 14), palette.textSecondary);
  wxBitmapBundle maxBundle = Theme::IconManager::GetIconBundle(
      Theme::SVG::MAXIMIZE, wxSize(14, 14), palette.textSecondary);
  wxBitmapBundle closeBundle = Theme::IconManager::GetIconBundle(
      Theme::SVG::CLOSE, wxSize(14, 14), palette.textSecondary);

  m_minBtn =
      new wxBitmapButton(m_topHeaderPanel, wxID_ANY, minBundle,
                         wxDefaultPosition, wxSize(30, 30), wxBORDER_NONE);
  m_minBtn->SetBackgroundColour(palette.sidebarBg);
  m_minBtn->SetToolTip(L"最小化");

  m_maxBtn =
      new wxBitmapButton(m_topHeaderPanel, wxID_ANY, maxBundle,
                         wxDefaultPosition, wxSize(30, 30), wxBORDER_NONE);
  m_maxBtn->SetBackgroundColour(palette.sidebarBg);
  m_maxBtn->SetToolTip(L"最大化 / 还原");

  m_closeBtn =
      new wxBitmapButton(m_topHeaderPanel, wxID_ANY, closeBundle,
                         wxDefaultPosition, wxSize(30, 30), wxBORDER_NONE);
  ...
  m_contentContainer = new wxPanel(this, wxID_ANY, wxDefaultPosition,
  ...
  // 实例化各子视图
  m_textView =
      new TextTranslationView(m_contentContainer, m_translationService);
  m_ocrView = new OcrTranslationView(m_contentContainer, m_translationService,
                                     m_ocrService);
  m_historyView = new PlaceholderView(m_contentContainer, L"翻译历史记录");
  m_settingsView =
      new SettingsView(m_contentContainer, m_translationService, m_ocrService);
  ...
  m_sidebar->Bind(Components::EVT_SIDEBAR_NAV_CHANGED, &MainFrame::OnNavChanged,
                  this);

  // 绑定窗口控制按钮事件
  m_minBtn->Bind(wxEVT_BUTTON, [this](wxCommandEvent &) { Iconize(true); });
  m_maxBtn->Bind(wxEVT_BUTTON,
                 [this](wxCommandEvent &) { Maximize(!IsMaximized()); });
  ...
  m_topHeaderPanel->Bind(wxEVT_LEFT_DCLICK, &MainFrame::OnHeaderDoubleClick,
                         this);
