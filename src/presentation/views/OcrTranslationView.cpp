...
void OcrTranslationView::InitUI() {
  ...
  wxBitmapBundle titleBundle = Theme::IconManager::GetIconBundle(
      Theme::SVG::OCR, wxSize(24, 24), palette.accentPrimary);
  ...
  // Top Overlay Bar (Preview & Replace Buttons when image is loaded)
  m_topOverlayBar =
  ...
  m_previewBtn = new Components::CustomButton(
      m_topOverlayBar, wxID_ANY, L"预览", Components::ButtonStyle::Secondary,
      wxDefaultPosition, wxSize(76, 30));
  m_previewBtn->SetIcon(Theme::SVG::EYE, wxSize(14, 14), palette.textPrimary);

  m_replaceBtn = new Components::CustomButton(
      m_topOverlayBar, wxID_ANY, L"替换", Components::ButtonStyle::Primary,
      wxDefaultPosition, wxSize(76, 30));
  m_replaceBtn->SetIcon(Theme::SVG::REPLACE, wxSize(14, 14), *wxWHITE);
  ...
  m_centerUploadBtn = new Components::CustomButton(
      m_dropzonePanel, wxID_ANY, L" 替换图片", Components::ButtonStyle::Primary,
      wxDefaultPosition, wxSize(130, 38));
  m_centerUploadBtn->SetIcon(Theme::SVG::CLOUD_UPLOAD, wxSize(16, 16),
                             *wxWHITE);
  m_centerUploadBtn->Hide();
  dropSizer->Add(m_centerUploadBtn, 0, wxALIGN_CENTER_HORIZONTAL);

  wxBitmapBundle uploadBundle = Theme::IconManager::GetIconBundle(
      Theme::SVG::CLOUD_UPLOAD, wxSize(44, 44), palette.accentPrimary);
  ...
  // Custom Paint Card (Image Preview / Dashed Prompt)
  ...
  m_copyBtn = new Components::CustomButton(m_resultCard, wxID_ANY, L"",
  ...
  m_clearBtn = new Components::CustomButton(m_resultCard, wxID_ANY, L"",
                                            Components::ButtonStyle::Secondary,
                                            wxDefaultPosition, wxSize(34, 34));
  ...
  m_recognizeBtn = new Components::CustomButton(
      this, wxID_ANY, L"识别", Components::ButtonStyle::Primary,
      wxDefaultPosition, wxSize(145, 42));
  m_recognizeBtn->SetIcon(Theme::SVG::OCR, wxSize(16, 16), *wxWHITE);

  m_stopBtn = new Components::CustomButton(this, wxID_ANY, L"停止",
  ...
  m_dropzonePanel->Bind(wxEVT_LEFT_DOWN,
                        &OcrTranslationView::OnSelectImageClicked, this);
  ...
  m_dropzonePanel->Bind(wxEVT_ENTER_WINDOW,
                        &OcrTranslationView::OnDropzoneMouseEnter, this);
  m_dropzonePanel->Bind(wxEVT_LEAVE_WINDOW,
                        &OcrTranslationView::OnDropzoneMouseLeave, this);
  ...
  m_previewBtn->Bind(wxEVT_BUTTON, &OcrTranslationView::OnPreviewClicked, this);
  m_replaceBtn->Bind(wxEVT_BUTTON, &OcrTranslationView::OnReplaceClicked, this);
  m_centerUploadBtn->Bind(wxEVT_BUTTON, &OcrTranslationView::OnReplaceClicked,
                          this);

  m_recognizeBtn->Bind(wxEVT_BUTTON, &OcrTranslationView::OnRecognizeClicked,
                       this);
...
void OcrTranslationView::OpenImageDialog() {
  wxFileDialog openFileDialog(this, L"选择待识别图片", "", "",
                              "Image Files "
                              "(*.png;*.jpg;*.jpeg;*.bmp;*.webp)|*.png;*.jpg;*."
                              "jpeg;*.bmp;*.webp|All Files (*.*)|*.*",
...
void OcrTranslationView::OnPreviewClicked(wxCommandEvent &WXUNUSED(event)) {
  if (m_loadedImage.IsOk()) {
    Components::ImagePreviewDialog dialog(this, m_loadedImage,
                                          L"图片预览 - " + m_imageFileName);
    dialog.ShowModal();
  }
}

void OcrTranslationView::OnReplaceClicked(wxCommandEvent &WXUNUSED(event)) {
  if (m_currentState == OcrTaskState::Recognizing)
    return;
  OpenImageDialog();
}

void OcrTranslationView::OnDropzoneMouseEnter(wxMouseEvent &event) {
  if (m_currentState == OcrTaskState::Recognizing) {
    event.Skip();
    return;
  }
...
void OcrTranslationView::OnDropzoneMouseLeave(wxMouseEvent &event) {
  if (m_dropzonePanel) {
...
void OcrTranslationView::UpdateDropzoneUI() {
...
void OcrTranslationView::OnRecognizeClicked(wxCommandEvent &WXUNUSED(event)) {
  ...
  if (!m_ocrService || !m_ocrService->IsModelLoaded()) {
    wxMessageBox(L"OCR "
                 L"模型尚未配置或未能成功加载！\n\n请先前往「系统设置」界面，配"
                 L"置合法的 PaddleOCR-VL 主模型与 mmproj 视觉投影器文件路径。",
                 L"OCR 服务未配置提示", wxOK | wxICON_WARNING, this);
    return;
  }
  ...
  m_ocrService->RecognizeImageStream(
      ...
      [this](const std::string &token) {
        ...
        wxTheApp->CallAfter([this, wToken]() {
          ...
          m_charCountText->SetLabel(
              wxString::Format(L"%zu 字符", current.Length()));
        });
      },
      [this](const std::string &fullText, bool success,
