...
void TextTranslationView::InitUI() {
  ...
  wxBitmapBundle titleBundle = Theme::IconManager::GetIconBundle(
      Theme::SVG::TEXT, wxSize(22, 22), palette.accentPrimary);
  ...
  m_statusBadge = new wxPanel(this, wxID_ANY, wxDefaultPosition, wxSize(78, 28),
                              wxBORDER_NONE);
  ...
  wxBitmapBundle infoBundle = Theme::IconManager::GetIconBundle(
      Theme::SVG::INFO, wxSize(16, 16), palette.textSecondary);
  ...
  m_instantTransBtn = new Components::CustomButton(
      m_bannerPanel, wxID_ANY, L"立即翻译", Components::ButtonStyle::Primary,
  ...
  // 事件绑定
  m_translateBtn->Bind(wxEVT_BUTTON, &TextTranslationView::OnTranslateClicked,
                       this);
  m_stopBtn->Bind(wxEVT_BUTTON, &TextTranslationView::OnStopClicked, this);
  m_instantTransBtn->Bind(wxEVT_BUTTON,
                          &TextTranslationView::OnTranslateClicked, this);
  ...
  m_copyBtn->Bind(wxEVT_BUTTON, &TextTranslationView::OnCopyTargetClicked,
                  this);
  m_sourceCard->GetTextCtrl()->Bind(
      wxEVT_TEXT, &TextTranslationView::OnSourceTextChanged, this);
}
...
void TextTranslationView::OnTranslateClicked(wxCommandEvent &WXUNUSED(event)) {
  ...
  m_translationService->ExecuteStreamTranslation(
      ...
      // 翻译完成/被中断回调
      [this](bool success, const std::string &fullText,
        ...
        if (success) {
          ...
          if (error == "已取消") {
            m_targetCard->GetTextCtrl()->AppendText(
                L"\n\n[⏹ 翻译已被用户中断]");
          } else {
