...
TextView::TextView(wxWindow* parent, std::shared_ptr<ModelManager> modelManager, wxWindowID id)
    ...
    m_modelManager->EnsureModelAsync(TargetModelType::Translation, BindUi([this](const std::string& statusMsg) {
                                     ...
                                     BindUi([this, text, srcLang, tgtLang](bool ok, const ServerStatusInfo& info) {
                                         if (!ok) {
                                             ...
                                             if (m_targetCard && m_targetCard->GetTextCtrl()) {
                                                 m_targetCard->GetTextCtrl()->SetValue(L"翻译模型未就绪: " + wxString::FromUTF8(info.message) +
