...
CustomInputBox::CustomInputBox(wxWindow* parent, wxWindowID id, const wxString& value, const wxString& hint, const wxPoint& pos, const wxSize& size, long style)
...
void CustomInputBox::InitUI(const wxString& value, const wxString& hint, long style) {
    auto palette = ThemeColors::GetCurrentPalette();
    SetBackgroundColour(palette.cardBg);

    m_mainSizer = new wxBoxSizer(wxVERTICAL);

    long innerStyle = (style & (wxTE_READONLY | wxTE_PASSWORD | wxTE_MULTILINE | wxTE_RICH2)) | wxBORDER_NONE;
    if (!m_isMultiline) {
        innerStyle |= wxTE_PROCESS_ENTER;
    }

    m_textCtrl = new wxTextCtrl(this, wxID_ANY, value, wxDefaultPosition, wxDefaultSize, innerStyle);
    if (!hint.IsEmpty()) {
...
void CustomInputBox::OnPaint(wxPaintEvent& WXUNUSED(event)) {
    ...
    if (!m_isMultiline && m_prefixSvg) {
        ...
        wxBitmapBundle iconBundle = IconManager::GetIconBundle(m_prefixSvg, wxSize(16, 16), iconColor);
        wxBitmap iconBmp = iconBundle.GetBitmap(m_prefixIconSize);
        if (iconBmp.IsOk()) {
            int iconX = 11_dip;
            int iconY = (size.y - m_prefixIconSize.y) / 2;
            gc->DrawBitmap(iconBmp, iconX, iconY, m_prefixIconSize.x, m_prefixIconSize.y);
        }
    }

    // 3. 绘制右侧快速清除 'x' 按钮 (仅单行非空时显示，亚像素级绝对居中与轻盈圆角微质感)
    if (!m_isMultiline && m_showClearButton && m_textCtrl && !m_textCtrl->GetValue().IsEmpty()) {
        wxRect clearRect = GetClearBtnRect();
        double cx = clearRect.x + clearRect.width / 2.0;
        double cy = clearRect.y + clearRect.height / 2.0;
        double circleRadius = clearRect.width / 2.0;

        if (m_isClearHovered) {
            bool isDark = (ThemeColors::GetInstance().GetCurrentTheme() == ThemeMode::Dark);
            wxColour hoverBg = isDark ? wxColour(255, 255, 255, 36) : wxColour(0, 0, 0, 18);
            gc->SetBrush(gc->CreateBrush(wxBrush(hoverBg)));
            gc->SetPen(*wxTRANSPARENT_PEN);
            gc->DrawEllipse(cx - circleRadius, cy - circleRadius, circleRadius * 2.0, circleRadius * 2.0);
        }

        wxColour clearColor = m_isClearHovered ? palette.accentPrimary : palette.textSecondary;
        wxGraphicsPen crossPen = gc->CreatePen(wxGraphicsPenInfo(clearColor).Width(1.6_dip).Cap(wxCAP_ROUND).Join(wxJOIN_ROUND));
        gc->SetPen(crossPen);
...
void CustomInputBox::OnMouseMove(wxMouseEvent& event) {
    ...
    if (!m_isMultiline && m_showClearButton && m_textCtrl && !m_textCtrl->GetValue().IsEmpty()) {
        insideClear = GetClearBtnRect().Inflate(2_dip, 2_dip).Contains(pt);
    }
...
void CustomInputBox::OnLeftDown(wxMouseEvent& event) {
    wxPoint pt = event.GetPosition();
    if (!m_isMultiline && m_showClearButton && m_textCtrl && !m_textCtrl->GetValue().IsEmpty() && GetClearBtnRect().Inflate(2_dip, 2_dip).Contains(pt)) {
        Clear();
