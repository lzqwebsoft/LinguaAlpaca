...
TranslationBubbleFrame::TranslationBubbleFrame(std::shared_ptr<ModelManager> modelManager, wxWindow* parent)
    ...
    m_speakBtnTimer.Bind(wxEVT_TIMER, [this](wxTimerEvent&) { UpdateSpeakBtnVisibility(); });
...
TranslationBubbleFrame::~TranslationBubbleFrame() {
    if (m_speakBtnTimer.IsRunning()) {
        m_speakBtnTimer.Stop();
    }
...
void TranslationBubbleFrame::InitUI() {
    ...
    wxBitmapBundle logoBundle = IconManager::GetIconBundle(SVG::TRANSLATE, wxSize(22, 22), palette.accentPrimary);
    ...
    m_titleText->SetFont(ThemeFont::GetFont(FontRole::WindowTitle));
    ...
    m_sourceToggleLabel->SetFont(ThemeFont::GetFont(FontRole::Control));
    m_sourceToggleLabel->SetCursor(wxCursor(wxCURSOR_HAND));

    m_sourcePreviewText = new wxStaticText(m_sourceToggleBar, wxID_ANY, "");
    m_sourcePreviewText->SetFont(ThemeFont::GetFont(FontRole::Caption));
    ...
    // 高性能底层绘制: 直接使用原生 GDI DC 绘制 1px 分隔线，避免每次 Paint
    // 创建重型 wxGraphicsContext COM 实例
    ...
    // 折叠栏点击
    auto onToggleClick = [this](wxMouseEvent&) { SetSourcePanelExpanded(!m_isSourceExpanded); };
    ...
    wxBitmapBundle speakBundle = IconManager::GetIconBundle(SVG::SPEAKER, wxSize(16, 16), palette.textSecondary);
    ...
    m_sourceCtrl->SetFont(ThemeFont::CreateFont(std::max(8, m_currentFontSize - 1), wxFONTWEIGHT_NORMAL));
    ...
    m_sourceSpeakBtn = new wxBitmapButton(m_sourcePanel, wxID_ANY, speakBundle, wxDefaultPosition, dip(26, 26), wxBORDER_NONE);
    ...
    m_targetCtrl->SetFont(ThemeFont::CreateFont(m_currentFontSize, wxFONTWEIGHT_NORMAL));
    ...
    m_targetSpeakBtn = new wxBitmapButton(m_targetPanel, wxID_ANY, speakBundle, wxDefaultPosition, dip(26, 26), wxBORDER_NONE);
    ...
    m_targetSpeakBtn->Hide();

    // 悬浮播放按钮自适应定位与置顶 (确保悬浮于 TextCtrl 之上且不遮挡滑动条)
    m_sourcePanel->Bind(wxEVT_SIZE, [this](wxSizeEvent& event) {
        event.Skip();
        RepositionSpeakButtons();
    });
    ...
    m_statusText->SetFont(ThemeFont::GetFont(FontRole::Caption));
    ...
    m_resizeGrip->Bind(wxEVT_LEFT_DOWN, [this](wxMouseEvent&) { StartResize(ResizeDirection::BottomRight, wxGetMousePosition(), m_resizeGrip); });
...
void TranslationBubbleFrame::ShowAndTranslate(const wxPoint& spawnPos, const std::string& sourceText) {
    ...
    if (hMon) {
        ...
        if (GetMonitorInfo(hMon, &mi)) {
            ...
            if (posX < mi.rcWork.left + 10_dip)
                posX = mi.rcWork.left + 10_dip;
            if (posY < mi.rcWork.top + 10_dip)
                posY = mi.rcWork.top + 10_dip;
        }
    }
#else
    int displayIdx = wxDisplay::GetFromPoint(wxPoint(posX, posY));
    if (displayIdx == wxNOT_FOUND) {
        displayIdx = wxDisplay::GetFromPoint(spawnPos);
    }
    if (displayIdx != wxNOT_FOUND) {
        ...
        if (posX < geom.GetLeft() + 10)
            posX = geom.GetLeft() + 10;
        if (posY < geom.GetTop() + 10)
            posY = geom.GetTop() + 10;
    }
#endif
...
void TranslationBubbleFrame::DoExecuteTranslation(const std::string& sourceText) {
    ...
    m_modelManager->ExecuteTranslationStream(task,
                                             // Token 流式接收
...
void TranslationBubbleFrame::SetSourcePanelExpanded(bool expanded) {
    ...
    if (m_isSourceExpanded) {
        if (m_sourceToggleIcon) {
            m_sourceToggleIcon->SetBitmap(IconManager::GetIconBundle(SVG::CHEVRON_DOWN, wxSize(16, 16), p.accentPrimary));
        }
        if (m_sourceToggleLabel) {
        ...
        if (m_sourceToggleIcon) {
            m_sourceToggleIcon->SetBitmap(IconManager::GetIconBundle(SVG::CHEVRON_RIGHT, wxSize(16, 16), p.textSecondary));
        }
        if (m_sourceToggleLabel) {
...
void TranslationBubbleFrame::UpdateSourcePreview() {
    if (!m_sourcePreviewText || m_isSourceExpanded)
        return;
    ...
    // 过滤与清洗：去除控制字符、换行符、Object Replacement Character (U+FFFC
    // 等)，并合并多余空格
    wxString cleaned;
    ...
    for (wxUniChar ch : raw) {
        ...
        if (ch == 0xFFFC || ch == 0xFFFD || ch == 0xFEFF || ch == 0x200B || ch == 0x200C || ch == 0x200D) {
        ...
        // 换行、制表符、空白字符
        if (ch == '\r' || ch == '\n' || ch == '\t' || ch == '\v' || ch == '\f' || ch == ' ' || ch == 0x00A0 || ch == 0x2028 || ch == 0x2029) {
            if (!lastWasSpace && !cleaned.IsEmpty()) {
                cleaned += ' ';
                lastWasSpace = true;
            }
...
void TranslationBubbleFrame::UpdateTheme() {
    ...
    if (m_sourceToggleBar)
        m_sourceToggleBar->SetBackgroundColour(palette.sidebarBg);
    ...
    auto updateBtn = [&palette](wxBitmapButton* btn, const char* svg, const wxColour& tint) {
        ...
        btn->SetBitmap(IconManager::GetIconBundle(svg, wxSize(19, 19), tint));
    };
...
void TranslationBubbleFrame::ProcessResizeDrag(const wxPoint& screenPos) {
    if (!m_isResizing || m_resizeDir == ResizeDirection::None)
        return;
    ...
    // 正交水平维度计算 (Left / Right)
    const bool isLeft = (m_resizeDir == ResizeDirection::Left || m_resizeDir == ResizeDirection::TopLeft || m_resizeDir == ResizeDirection::BottomLeft);
    const bool isRight = (m_resizeDir == ResizeDirection::Right || m_resizeDir == ResizeDirection::TopRight || m_resizeDir == ResizeDirection::BottomRight);
    ...
    // 正交垂直维度计算 (Top / Bottom)
    const bool isTop = (m_resizeDir == ResizeDirection::Top || m_resizeDir == ResizeDirection::TopLeft || m_resizeDir == ResizeDirection::TopRight);
    const bool isBottom = (m_resizeDir == ResizeDirection::Bottom || m_resizeDir == ResizeDirection::BottomLeft || m_resizeDir == ResizeDirection::BottomRight);
    ...
    // 仅在坐标或尺寸真正发生变化时才调用 SetSize，消除不必要的 Layout 与 WM_PAINT
    // 刷新
...
void TranslationBubbleFrame::HandleEdgeLeftDown(wxMouseEvent&, wxWindow* sourceWin, bool isHeader) {
    ...
    if (dir != ResizeDirection::None && (!isHeader || dir != ResizeDirection::Bottom)) {
        ...
        m_isDragging = true;
        // 计算屏幕绝对坐标相对窗口左上角的偏移量，彻底修复因点击子控件 (如
        // m_titleText) 导致的跳跃 Bug
...
void TranslationBubbleFrame::ApplyFontSize(int fontSize, bool saveToConfig) {
    ...
    if (clamped == m_currentFontSize && !saveToConfig)
        return;
...
void TranslationBubbleFrame::RepositionSpeakButtons() {
    const int rightMargin = 10_dip;
    const int topMargin = 4_dip;
    if (m_sourceSpeakBtn && m_sourcePanel) {
        wxSize sz = m_sourcePanel->GetClientSize();
        wxSize btnSz = m_sourceSpeakBtn->GetSize();
        int x = std::max(4_dip, sz.x - btnSz.x - rightMargin);
        m_sourceSpeakBtn->Move(x, topMargin);
        if (m_sourceSpeakBtn->IsShown()) {
            m_sourceSpeakBtn->Raise();
        }
    }
    if (m_targetSpeakBtn && m_targetPanel) {
        ...
        int x = std::max(4_dip, sz.x - btnSz.x - rightMargin);
        m_targetSpeakBtn->Move(x, topMargin);
        if (m_targetSpeakBtn->IsShown()) {
            m_targetSpeakBtn->Raise();
        }
    }
}

void TranslationBubbleFrame::UpdateSpeakBtnVisibility() {
    if (!IsShown()) {
        if (m_speakBtnTimer.IsRunning()) {
            m_speakBtnTimer.Stop();
        }
        if (m_sourceSpeakBtn && m_sourceSpeakBtn->IsShown()) {
            m_sourceSpeakBtn->Hide();
        }
        if (m_targetSpeakBtn && m_targetSpeakBtn->IsShown()) {
            m_targetSpeakBtn->Hide();
        }
        return;
    }
