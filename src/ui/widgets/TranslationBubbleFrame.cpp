...
void TranslationBubbleFrame::InitUI() {
    ...
    m_sourceSpeakBtn = new wxBitmapButton(m_sourcePanel, wxID_ANY, wxBitmapBundle(), wxDefaultPosition, dip(24, 24), wxBORDER_NONE);
    ...
    m_targetSpeakBtn = new wxBitmapButton(m_targetPanel, wxID_ANY, wxBitmapBundle(), wxDefaultPosition, dip(24, 24), wxBORDER_NONE);
    m_targetSpeakBtn->SetToolTip(L"朗读译文");
    m_targetSpeakBtn->SetCursor(wxCursor(wxCURSOR_HAND));

    // 悬浮播放按钮自适应定位 (去除了无谓反复调用 Raise() 引起的 Win32 Z-Order 刷新与闪烁)
